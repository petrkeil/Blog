...
which is the rate of failures (deaths) per unit time. It is the probability of a death occuring in the small interval of time between $t$ and $dt$.
...
or *hazard* or *instantaneous failure rate* or *age-specific failure rate*). Is the instantaneous rate of change in the log of the number of survivors per unit of time. It 
...
$$\lambda(t)=\lim_{\Delta t \to 0} \frac{Pr(t \leq T < (t + \Delta t))}{\Delta t} = 
\frac{f(t)}{S(t)}
=-\frac{S'(t)}{S(t)}$$
...
where both $\mu > 0$ and $t > 0$, and $\mu = 1/\lambda$. Note that $f(t)$ has an intercept at $1/\mu$ 
