...
# Continuous predictor of hazard in JAGS

I will use the `roaches` dataset from the Crawley's book.
...
## The `survreg` solution

Survival analysis using `survreg`, with `weight` as the predictor:
```{r}
...
Here I plot the resulting posterior distributions of the intercept and slope.
THe red vertical lines are estimates from the `survreg` function:
