---
title: "Reproducible psychedelic art with R: tribute to spatstat"
author: "Petr Keil"
output: 
  html_document: 
    highlight: tango
...
Loading the amazing `spatstat` library
...
```{r}
  png(filename = "airport_in_the_morning.png", width=6000, height=4000, res=400)
  par(mai=c(0,0,0,0))
  ...
  #contour(dist.from.all.lines, col="black", add=TRUE)
...
```{r}
  png(filename = "airport_at_night.png", width=6000, height=4000, res=400)
  par(mai=c(0,0,0,0))
  dist.from.all.lines.exp <- exp(-Lambda*dist.from.all.lines)
...
```{r}
png(filename = "midnight_eggs.png", width=6000, height=4000, res=400)
  par(mai=c(0,0,0,0))
...
```{r}
png(filename = "morning_eggs.png", width=6000, height=4000, res=400)
  par(mai=c(0,0,0,0))
 plot(rcdist, legend=FALSE, main="", frame=FALSE, box=FALSE, ribbon=FALSE,
